...
public class EmployeeTest {
	...
	public static void main(String[] args) {
		...
		emps[0] = new Admin("Dav", "P913652874", 50000,Branch.Taipei);
		emps[1] = new Admin("Amy", "B210987654", 50000,Branch.London);
		emps[2] = new Engineer("David", "C109876543", 60000,Branch.Paris);
		emps[3] = new Manager("Louis", "D124680135", 60000, "TW Sales",Branch.Tokyo);
		emps[4]= new Director("Nicole", "R202468135", 60000, "Global Sales", 1000000,Branch.Taipei);
...
//			System.out.println(emps[i]);
		...
		for(int i=0; i<emps.length; i++) {
			System.out.println(emps[i].getName()+": "+emps[i].getBranch().getCurrency()+emps[i].getPay());



			if(emps[i] instanceof RegularStaff) {
				System.out.println("尾牙抽獎 :"+RegularStaff.getLuckDraw() );
				System.out.println("年終獎金 :"+emps[i].getBranch().getCurrency()+((RegularStaff)emps[i]).getBonus()+"\n");				
			}else
